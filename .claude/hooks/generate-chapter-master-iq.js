...
function buildMissingPrompt(ch, repoRoot) {
  ...
  return [
    ...
    '',
    '## Table of Contents',
    '(auto-generated list linking to every section below — 9 sections total)',
    '',
    '---',
    '',
    ...
    'Create a visual memory aid using ASCII art and/or Mermaid diagrams so the',
    'reader can reconstruct the chapter concepts from memory at a glance.',
    '',
    'Required elements:',
    '  A) ASCII MIND MAP — a text-based tree showing all concepts and how they relate:',
    '     Format example:',
    '       [Chapter Topic]',
    '         ├── Concept A',
    '         │     ├── Sub-concept A1',
    '         │     └── Sub-concept A2 ⚠️ gotcha',
    '         ├── Concept B',
    '         │     └── Sub-concept B1',
    '         └── Concept C',
    '     Use ⚠️ to mark gotchas, ✅ for best practice, ❌ for things to avoid.',
    '',
    '  B) FLOWCHART (Mermaid) — decision logic or execution flow for the chapter',
    '     Use ```mermaid fenced block. Examples:',
    '     - Loop chapter: which loop type to pick (for/while/do-while)',
    '     - Arrays chapter: which method to call (push/splice/filter/find)',
    '     Format: flowchart TD or LR with decision diamonds and outcome boxes.',
    '     Keep it to max 15 nodes — clarity over completeness.',
    '',
    '  C) EXECUTION TRACE BOX — for the trickiest concept in the chapter,',
    '     show a step-by-step state table (like a debugger watch window):',
    ...
    'Write a single, complete LinkedIn post about this chapter that a JS learner',
    'or developer would genuinely want to share. This is NOT a summary — it is a',
    'standalone piece of content that doubles as a quick-revision card.',
    '',
    'STRICT REQUIREMENTS:',
    '  • Hook line: 1–2 punchy lines that stop the scroll (a surprising fact,',
    '    a bold claim, or a tricky question from the chapter)',
    '  • Length: 200–280 words — long enough to be valuable, short enough to read in 60s',
    '  • Structure: use line breaks liberally (no wall of text)',
    '    - Short punchy sentences',
    '    - Bullet-point lists with ✅ / ❌ / ⚠️ / 💡 emojis where helpful',
    '    - At least one code snippet (keep it ≤ 5 lines, use ``` fences)',
    '  • Cover the single MOST important gotcha or insight from the chapter',
    '    (the thing most developers get wrong)',
    '  • Include a "Key Takeaway" line in bold',
    '  • End with 3–5 relevant hashtags',
    '  • Tone: conversational, confident, peer-to-peer — not a tutorial, not a lecture.',
    '    Write as if a senior dev is sharing a lesson they learned the hard way.',
    '',
    'FORMAT INSIDE THE MD FILE:',
    '  Wrap the post in a blockquote (>) with a heading:',
    '  ### 📢 LinkedIn Post',
    '  > [post content here with line breaks preserved]',
    '',
    '└──────────────────────────────────────────────────────────────────────┘',
    '',
    ...
    '  • Memory map ASCII art must be in a plain ``` (no language tag) fenced block',
    '  • Mermaid diagrams must use ```mermaid fenced blocks',
    '  • LinkedIn post must use the ### 📢 LinkedIn Post + > blockquote format',
