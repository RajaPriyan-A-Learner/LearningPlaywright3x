...
import { hasApiKey } from '../ai/config/provider';
...
class CustomTTAReporter implements Reporter {
    ...
    private snapshotAndLoadPrev(): { prev?: BuildSummary; curr: BuildSummary } {
        ...
        if (lastSnapshot) {
            try {
                prev = JSON.parse(
                    fs.readFileSync(path.join(dir, lastSnapshot), 'utf-8'),
                ) as BuildSummary;
            } catch {
    ...
    private associateLogsWithSteps(_test: TestCase, result: TestResult, testSteps: StepData[], allLogs: string[]): void {
        ...
        for (const attachment of result.attachments) {
            ...
            if (logMatch && logMatch[1] && attachment.contentType === 'text/plain') {
                const stepIndex = parseInt(logMatch[1], 10);
                const targetStep = testSteps[stepIndex];
                if (targetStep) {
                    let logContent = '';
                    if (attachment.body) {
        ...
        for (let logIndex = 0; logIndex < allLogs.length; logIndex++) {
            ...
            for (let stepIndex = 0; stepIndex < testSteps.length; stepIndex++) {
                ...
                if (pattern && step && pattern.test(log)) {
                ...
                for (let stepIdx = 0; stepIdx < testSteps.length && logIdx < unassignedLogs.length; stepIdx++) {
                    ...
                    if ((step.consoleLogs?.length ?? 0) === 0) {
                        for (let i = 0; i < logsForThisStep; i++) {
                            const nextLog = unassignedLogs[logIdx++];
                            if (nextLog !== undefined) {
                                step.consoleLogs?.push(nextLog);
                            }
                        }
                    }
                }
...
<body>
    ...
    private generateSelfHealTab(): string {
        ...
        return `<div class="ai-data-list">${this.healReports.map(({ test, report }) => {
            const verified = report.verified && report.verified.length > 0
            ...
            const rejected = report.rejected && report.rejected.length > 0
    ...
    private generateFlakyTab(): string {
        ...
        const flakyList = r.flakyTests.length
            ? r.flakyTests.map((t) => `<div class="flaky-item">🔁 ${this.escapeHtml(t)}</div>`).join('')
